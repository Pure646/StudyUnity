...
{
    Hero_Ctrl m_RefHero = null;         // 주인공 객체의 참조 변수
    ...
    void Start()
    {
        m_RefHero = transform.root.GetComponent<Hero_Ctrl>();
        // = GameObject.FindObjectOfType<Hero_Ctrl>();
    ...
    void Update()
    {
        ...
        transform.position = Parent_Pos +
            new Vector3(Mathf.Cos(angle * Mathf.Deg2Rad) * radius, Mathf.Sin(angle * Mathf.Deg2Rad) * radius, 0.0f);
        // 반시계 방향
        ...
        if (m_ShootCool <= 0.0f)
        {
            ...
            if(IsDouble == true)            // 더블샷
            {
                ...
                for(int i = 0; i < 2; i++)
                {
                    ...
                    a_Pos.y += 0.2f - (i * 0.4f);
                    m_CloneObj.transform.position = a_Pos;
                }
            }
            else
            {
