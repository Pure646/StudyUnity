...
public class Monster_Ctrl : MonoBehaviour
{
    ...
    void Boss_AI_Update()
    {
        ...
        {
            m_CurPos = transform.position;
            float a_ArrivePos = CamResol.m_VpWMax.x - 1.9f; //도착 위치
            if (a_ArrivePos < m_CurPos.x)
            {
                m_CurPos.x += (-1.0f * Time.deltaTime * m_Speed);   //왼쪽으로 이동
                if (m_CurPos.x <= a_ArrivePos)  //도착 위치까지 도착했을 때
                {
                    shoot_Time = 1.28f;
        ...
        else if (m_BossState == BossState.BS_FEVER_ATT)  //피버 공격
        {
            ...
            if (shoot_Time <= 0.0f)
            {
                ...
                for (float Angle = 0.0f; Angle < 360.0f; Angle += 15)
                {
                    ...
                    //--- 총알이 날아가는 방향으로 회전시키기...
                    a_BulletSc.transform.right = new Vector3(-a_TargetV.x, -a_TargetV.y, 0.0f);
                    //-a_TargetV 마이너스를 대입한 이유는 Flip x 에 체크되어 있기 때문에...
                    //--- 총알이 날아가는 방향으로 회전시키기...
