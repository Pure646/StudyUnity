...
public class HomingMs_Ctrl : MonoBehaviour
{
    ...
    float m_RotSpeed = 250.0f;  //회전속도

    //--- 유도탄 변수
    [HideInInspector] public GameObject Target_Obj = null;  //타겟 참조 변수
    Vector3 m_DesireDir;    //타겟을 향하는 방향 변수
    //--- 유도탄 변수
    ...
    void Update()
    {
        ...
        if (Target_Obj != null)
        {
            ...
            { //총알이 화면을 벗어나면...
                Monster_Ctrl a_RefMon = Target_Obj.GetComponent<Monster_Ctrl>();
                if (a_RefMon != null && a_RefMon.m_HomingLockOn == this.gameObject)
                    a_RefMon.m_HomingLockOn = null;
            }
        }//if (Target_Obj != null)
